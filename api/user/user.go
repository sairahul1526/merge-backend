...
func UserLogin(w http.ResponseWriter, r *http.Request) {
	...
	fieldCheck := UTIL.RequiredFiledsCheck(body, CONSTANT.UserLoginUserRequiredFields)
	if len(fieldCheck) > 0 {
	...
	user, err := DB.MainDB.SelectSQL(CONSTANT.UsersTable, []string{"id", "name", "email", "status"}, map[string]string{"email": body["email"], "password": UTIL.GetMD5HashString(body["password"])})
	if err != nil {
	...
	accessToken, err := UTIL.CreateJWTToken(map[string]interface{}{"user_id": user[0]["id"], "role": CONSTANT.UserNormal}, CONSTANT.UserJWTAccessExpiry, false)
	if err != nil {
		UTIL.SetReponse(w, CONSTANT.StatusCodeServerError, "", CONSTANT.ShowDialog, err.Error(), response)
		return
	}
	refreshToken, err := UTIL.CreateJWTToken(map[string]interface{}{"user_id": user[0]["id"], "role": CONSTANT.UserNormal}, CONSTANT.UserJWTRefreshExpiry, true)
	if err != nil {
		UTIL.SetReponse(w, CONSTANT.StatusCodeServerError, "", CONSTANT.ShowDialog, err.Error(), response)
		return
	}
...
func UserSignUp(w http.ResponseWriter, r *http.Request) {
	...
	// check for required fields
	fieldCheck := UTIL.RequiredFiledsCheck(body, CONSTANT.UserSignUpUserRequiredFields)
	if len(fieldCheck) > 0 {
		UTIL.SetReponse(w, CONSTANT.StatusCodeBadRequest, fieldCheck+" required", CONSTANT.ShowDialog, "", response)
		return
	}
	...
	// refresh token - jwt token with long expiry to get new access token if expired
	// if refresh token expired, need to login
	accessToken, err := UTIL.CreateJWTToken(map[string]interface{}{"user_id": userID, "role": CONSTANT.UserNormal}, CONSTANT.UserJWTAccessExpiry, false)
	if err != nil {
		UTIL.SetReponse(w, CONSTANT.StatusCodeServerError, "", CONSTANT.ShowDialog, err.Error(), response)
		return
	}
	refreshToken, err := UTIL.CreateJWTToken(map[string]interface{}{"user_id": userID, "role": CONSTANT.UserNormal}, CONSTANT.UserJWTRefreshExpiry, true)
	if err != nil {
		UTIL.SetReponse(w, CONSTANT.StatusCodeServerError, "", CONSTANT.ShowDialog, err.Error(), response)
		return
	}
...
func UserRefreshToken(w http.ResponseWriter, r *http.Request) {
	...
	accessToken, err := UTIL.CreateJWTToken(map[string]interface{}{"user_id": r.Header.Get("user_id"), "role": CONSTANT.UserNormal}, CONSTANT.UserJWTAccessExpiry, false)
	if err != nil {
		UTIL.SetReponse(w, CONSTANT.StatusCodeServerError, "", CONSTANT.ShowDialog, err.Error(), response)
		return
	}
